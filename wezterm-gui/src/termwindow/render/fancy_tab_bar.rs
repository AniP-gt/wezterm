...
impl crate::TermWindow {
    ...
    fn build_horizontal_fancy_tab_bar(
        ...
        for item in items {
            match item.item.clone() {
                ...
                TabBarItem::Workspace { .. } => left_eles.push(item_to_elem(item)),
        ...
        for item in items {
            match item.item.clone() {
                TabBarItem::LeftStatus | TabBarItem::RightStatus | TabBarItem::None => {
                    ...
                    self.constrain_vertical_tab_width(&mut elem, &metrics, tab_bar_width);
                    tab_eles.push(elem);
                }
                TabBarItem::Workspace { .. } => {
                    let mut elem = self.tab_item_to_element(
                        item,
                        &font,
                        &metrics,
                ...
                TabBarItem::Tab { tab_idx, active } => {
                    ...
                    elem.display = DisplayType::Block;
                    elem.vertical_align = VerticalAlign::Top;
                    elem.margin.left = Dimension::Cells(1.0);
                    self.constrain_vertical_tab_width(&mut elem, &metrics, tab_bar_width);
    ...
    fn constrain_vertical_tab_width(
        &self,
        elem: &mut Element,
        metrics: &RenderMetrics,
        tab_bar_width: f32,
    ) {
        let width_context = |pixel_max| DimensionContext {
            dpi: self.dimensions.dpi as f32,
            pixel_max,
            pixel_cell: metrics.cell_size.width as f32,
        };

        let horizontal = |dim: &BoxDimension| {
            dim.left.evaluate_as_pixels(width_context(tab_bar_width))
                + dim.right.evaluate_as_pixels(width_context(tab_bar_width))
        };

        let content_width = (tab_bar_width
            - horizontal(&elem.margin)
            - horizontal(&elem.padding)
            - horizontal(&elem.border))
        .max(0.0);

        elem.min_width = Some(Dimension::Pixels(content_width));
        elem.max_width = Some(Dimension::Pixels(content_width));
    }
